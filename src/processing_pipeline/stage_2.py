...
def extract_snippet_clip(input_file, output_file, formatted_start_time, formatted_end_time):
...
@task(log_prints=True, retries=3)
def insert_new_snippet_to_snippets_table_in_supabase(supabase_client, audio_file_id, file_path, file_size):
    supabase_client.insert_snippet(audio_file_id, file_path, file_size)


@task(log_prints=True)
def process_llm_response(supabase_client, llm_response, local_file, s3_client, r2_bucket_name):
    try:
        print(f"Processing llm response {llm_response['id']}")
        flagged_snippets = llm_response["flagged_snippets"]
        for snippet in flagged_snippets:
            start_time = snippet["start_time"]
            end_time = snippet["end_time"]
            output_file = f"snippet_{snippet['id']}.mp3"
            parts = local_file.split("_")
            folder_name = f"{parts[0]}_{parts[1]}"

            extract_snippet_clip(local_file, output_file, start_time, end_time)
            ...
            insert_new_snippet_to_snippets_table_in_supabase(
                supabase_client, llm_response["audio_file"]["id"], uploaded_path, file_size
            )
...
def audio_clipping(repeat):
    ...
    while True:
        ...
        if llm_response:
            ...
            process_llm_response(supabase_client, llm_response, local_file, s3_client, R2_BUCKET_NAME)
