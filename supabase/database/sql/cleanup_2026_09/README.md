...
| 15 | `15_requeue_stuck_retryable_errors.sql` | DDL (log table) + yes + cron | **NOT RUN** (VER-389, PR #118). Step 0 audit table `snippet_requeue_log`; step 2 re-queues the 101 election-tagged 95+ `Error` rows since Aug plus the 4 hand-held Fulton rows, all to `New` (full Stage 3 re-run; batch `requeue-2026-09-21-ver389-step2`); step 3 schedules `sweep_retryable_errors(50)` hourly 08-23 UTC. Rollback inside the file, scoped to the log |
