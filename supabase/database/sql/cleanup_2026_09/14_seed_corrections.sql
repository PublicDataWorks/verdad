...
-- production on 2026-09-17 20:35 UTC, from the CodeRabbit review of PR #98. Idempotent; each statement is a no-op
-- once applied. Tracking: VER-326, VER-375.
--
...
--    standing. Split: the existing entry keeps only the meeting and the PBS source; a new entry carries the
--    candidacy with its two 'other' sources (context only until a tier1/tier2 source is attached).
...
moved AS (
    ...
    SET kb_entry = (SELECT id FROM candidacy)
    FROM combined c
...
-- The new candidacy entry has no embedding; run the paged embedding backfill (VER-375/VER-377) afterwards.
-- Verify:
-- SELECT e.fact, e.confidence_score, count(s.id) AS sources FROM public.kb_entries e
--   LEFT JOIN public.kb_entry_sources s ON s.kb_entry = e.id
