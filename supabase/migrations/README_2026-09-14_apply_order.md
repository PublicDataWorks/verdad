...
The files carry full 14-digit versions (`20260915000100` … `20260915000400`) because the
...
| # | File | Notes |
|---|------|-------|
| 1 | `20260915000100_like_count_columns_and_trigger.sql` | Idempotent; a no-op on production (records what is already live). Required on fresh environments before step 4. |
...
| 4 | `20260915000400_get_snippets_include_count.sql` | Drops the old `get_snippets(text,jsonb,int,int,text,text)` overload and creates the new one with `p_include_count boolean DEFAULT true`; re-grants; `NOTIFY pgrst, 'reload schema'`. Existing callers that do not send `p_include_count` are unaffected. |
