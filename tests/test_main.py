...
class TestMain:
    ...
    def test_client_initialization(self, mock_environment):
        ...
        with patch("boto3.client") as mock_boto3, patch("dotenv.load_dotenv"):
    ...
    def test_main_execution_success(self, mock_environment):
        ...
        with patch("boto3.client") as mock_boto3, patch(
            "processing_pipeline.timestamped_transcription_generator.TimestampedTranscriptionGenerator"
        ) as mock_generator, patch("os.path.exists", return_value=True), patch("os.remove") as mock_remove, patch(
            "builtins.print"
        ) as mock_print, patch(
            "dotenv.load_dotenv"
        ):
            ...
            mock_generator.run.assert_called_once_with(
                "radio_1853b3_20241127_102353.mp3", mock_environment["GOOGLE_GEMINI_KEY"], 10
            )
    ...
    def test_main_execution_file_not_found(self, mock_environment):
        ...
        with patch("boto3.client") as mock_boto3, patch("os.path.exists", return_value=False) as mock_exists, patch(
            "os.remove"
        ) as mock_remove, patch("builtins.print") as mock_print, patch("dotenv.load_dotenv"):
            ...
            mock_exists.assert_has_calls(
                [call("radio_1853b3_20241127_102353.mp3"), call("radio_1853b3_20241127_102353.mp3")]
            )
    ...
    def test_main_execution_with_download_error(self, mock_environment):
        ...
        with patch("boto3.client") as mock_boto3, patch("os.path.exists", return_value=False) as mock_exists, patch(
            "os.remove"
        ) as mock_remove, patch("dotenv.load_dotenv"):
    ...
    def test_main_execution_with_cleanup_error(self, mock_environment):
        ...
        with patch("boto3.client") as mock_boto3, patch(
            "processing_pipeline.timestamped_transcription_generator.TimestampedTranscriptionGenerator"
        ) as mock_generator, patch("os.path.exists", return_value=True), patch(
            "os.remove", side_effect=Exception("Cleanup failed")
