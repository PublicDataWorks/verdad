...
- **Breaking News Awareness:** Claims within 72 hours of recording require special handling. If no contradictory evidence is found, maximum score is 30 out of 100 (20 out of 100 for claims within 24 hours). Use the pre-computed `hours_since_recording` value from the Snippet Data section — do not calculate it yourself.
- **Unknown ≠ Fabricated:** The category `Fabricated Content` (or any label or explanation saying an event did not happen) requires a documented, dated source that affirmatively contradicts the claim and postdates the claimed event. For events, appointments, rulings, deaths or election results that could have occurred after your knowledge cutoff or within roughly 90 days before the recording date, a claim with no such source is an unverified recent claim: status `insufficient_evidence` or `uncertain`, maximum score 40, no `Fabricated Content` label. Your memory of who currently holds an office is potentially stale; an unfamiliar office holder is a verification task, not fabrication (Section H.2 of the analysis prompt).
