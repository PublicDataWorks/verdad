...
class Dumper:
    ...
    def header(self, counts_placeholder: str) -> None:
        ...
        self.out.append("-- actually runs, not a replay of the repo's history: the 29 versions in")
        self.out.append("-- supabase_migrations.schema_migrations are comment-only placeholder files next")
        self.out.append("-- to this one (most were applied out-of-band and their SQL never landed in git;")
        self.out.append("-- the rest are superseded by this file, bodies in git history), and the files")
        self.out.append("-- in supabase/database/sql/ are historical.")
        self.out.append("--")
        ...
        self.out.append("")
        self.out.append("-- Empty databases only: refuse where the schema already exists (production, or")
        self.out.append("-- any database past this version).")
        self.out.append("DO $$")
        self.out.append("BEGIN")
        self.out.append("    IF to_regclass('public.snippets') IS NOT NULL THEN")
        self.out.append("        RAISE EXCEPTION 'public.snippets already exists: do not run the baseline here; "
                        "run: supabase migration repair --linked --status applied 20260915000000';")
...
def applied_versions(client: Client) -> str:
    """The manifest tests/test_migrations.py checks the migration files against."""
    rows = client.query(
...
def main() -> int:
    ...
    parser.add_argument(
        "--applied-versions",
        action="store_true",
        help="instead of the schema, list supabase_migrations.schema_migrations (version<TAB>name per line)",
    )
    args = parser.parse_args()
    client = Client(args.project_ref, args.api_url)
    if args.applied_versions:
        sql = applied_versions(client)
    else:
        schemas = [s.strip() for s in args.schemas.split(",") if s.strip()]
        sql = Dumper(client, schemas).run()
    if args.output:
